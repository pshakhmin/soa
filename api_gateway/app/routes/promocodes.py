...
def handle_grpc_error(e: grpc.RpcError):
    ...
    if e.code() == grpc.StatusCode.NOT_FOUND:
        ...
        raise HTTPException(
            ...
            detail="An internal error occurred with the promocode service.",
        )
...
async def create_promocode(
    promocode_in: api_schemas.PromocodeCreate,
    ...
    logger.info(
        f"User {current_user.id} attempting to create promocode: {promocode_in.code}"
    )
    try:
        grpc_request = promocode_pb2.CreatePromocodeRequest(
            code=promocode_in.code,
            discount_type=promocode_pb2.DiscountType.Value(
                promocode_in.discount_type.name
            ),
        ...
        response_data = api_schemas.PromocodeResponseData.from_orm(
            grpc_response.promocode
        )
        ...
        logger.exception(
            f"Unexpected error creating promocode for user {current_user.id}: {e}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_promocode(
    ...
    try:
        ...
        logger.exception(
            f"Unexpected error getting promocode {promocode_id} for user {current_user.id}: {e}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="An unexpected error occurred.",
...
async def list_promocodes(
    ...
    logger.info(
        f"User {current_user.id} attempting to list promocodes (page={page}, size={page_size})"
    )
    try:
        grpc_request = promocode_pb2.ListPromocodesRequest(
        ...
        promocodes_data = [
            api_schemas.PromocodeResponseData.from_orm(p)
            for p in grpc_response.promocodes
        ]
        ...
        logger.exception(
            f"Unexpected error listing promocodes for user {current_user.id}: {e}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="An unexpected error occurred.",
...
async def update_promocode(
    promocode_id: uuid.UUID,
    promocode_in: api_schemas.PromocodeUpdate,
    stub: promocode_pb2.PromocodeServiceStub = Depends(get_promocode_stub),
    current_user: api_schemas.UserResponse = Depends(get_current_business_user),
):
    logger.info(
        f"User {current_user.id} attempting to update promocode: {promocode_id}"
    )
    try:
        ...
        if "discount_type" in update_fields:
            grpc_request.discount_type = promocode_pb2.DiscountType.Value(
                update_fields["discount_type"].name
            )
        ...
        logger.exception(
            f"Unexpected error updating promocode {promocode_id} for user {current_user.id}: {e}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="An unexpected error occurred.",
...
async def delete_promocode(
    ...
    logger.info(
        f"User {current_user.id} attempting to delete promocode: {promocode_id}"
    )
    try:
        ...
        logger.exception(
            f"Unexpected error deleting promocode {promocode_id} for user {current_user.id}: {e}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="An unexpected error occurred.",
